...
namespace AtendimentoMultiTenant.Web.ManagementArea.Pages.Feature.List
{
	public class FeatureHandler : HandlerBase, IHandler<FeatureRequest, FeaturePagedRequest, FeatureResponse>, IFeatureHandler
	{
		...
		public async Task<ResponseFactory<FeatureResponse>> Delete(Guid id, string type)
		{
			ResponseFactory<FeatureResponse>? result = new()!;

			try
			{
				var requestMessage = new HttpRequestMessage(HttpMethod.Delete, $"Api/Feature/Delete/{id}/{type}");
				requestMessage.Headers.Authorization = new AuthenticationHeaderValue("Bearer", await GetToken());
				requestMessage.Headers.Accept.Add(new MediaTypeWithQualityHeaderValue("application/json"));
				...
				if (response.IsSuccessStatusCode)
				{
					result = JsonConvert.DeserializeObject<ResponseFactory<FeatureResponse>?>(response.Content.ReadAsStringAsync().Result);
				}
			}
			catch (Exception)
			{
